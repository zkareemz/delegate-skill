...
pi did the typing; you own the judgment. This is where delegation earns its keep or quietly ships a
...
`result.json`'s `finalMessage` carries pi's own claim that the gates passed. Treat that as a claim, not
evidence — re-run the project's actual test/lint/build commands in the working tree and read the
output. A run that "passed" in pi's report but fails when you run it is exactly the failure this step
exists to catch, and it happens often enough to be worth the minute every time.
...
for. If `dirtyBefore` was `true`, first separate pi's edits from changes that were already in the tree.

- **Scope creep** — did pi change things the brief said to leave untouched? Unasked refactors, renames,
...
- **Quiet judgment calls** — sometimes pi makes a defensible decision the brief didn't anticipate. Don't
  just accept it because it looks reasonable; understand it and decide.

## The commit boundary

When the gates pass and the diff holds, **you commit** — the orchestrator, never pi. The brief instructs
pi not to commit, but pi *can* run `git` via its shell tool, so confirm it didn't (`git log -1`,
`git status`) before you write your own commit. Committing should be the act of the party that verified
the work. Write a clear message describing what landed. If your project attributes co-authorship, that's
the place for it.

## Reworking: send the delta, not the whole task

If the review turns up problems, don't restate the entire brief. Continue the same pi session with just
the correction:
...
(`<id>` is the `sessionId` from the first run's `result.json`.) Reusing the session keeps pi's context
from the first run, so a short delta is enough. Then review again — rework gets the same gate-rerun and
diff-read as the original, no shortcuts. Repeat until it's right, then commit.
...
- **Report design decisions** pi made, and any defensible-but-unrequested turns it took.
...
- **Stop and ask** if correct completion requires going beyond the brief — don't expand the mandate on
  your own. A scope change is the human's call, not yours or pi's.
