...
task's brief** as an explicit line. pi has no memory of the earlier run, so a constraint that emerged in
task 2 must be restated in task 5's brief or it won't hold. This is the queue equivalent of keeping
briefs self-contained. (If two tasks are genuinely a continuation, you can instead resume the same pi
session with `--session-id <id> --resume` — but an explicit restated constraint is more robust than
relying on session memory across a long queue.)
...
- **"Needs your eyes"** — design decisions pi made, non-blocking nitpicks, anything you want the human
