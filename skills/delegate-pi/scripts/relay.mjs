...
function dispatch(adapter2, ctx, writeResult) {
  const baseArgv = ctx.opts.resume ? adapter2.buildResumeArgv(ctx) : adapter2.buildArgv(ctx);
  const argv = adapter2.sendBrief === "argv" ? [...baseArgv, ctx.briefText] : baseArgv;
  const child = spawn(adapter2.name, argv, {
    cwd: ctx.opts.cd,
...
`, "utf8");
    ...
    const finalMessage = adapter2.parseFinalMessage(events);
    const sessionId = adapter2.parseSessionId(events) ?? ctx.sessionId;
    const result = writeResult({
...
// src/engine/index.ts
var adapter = piAdapter;
var HELP = `delegate-skill relay \u2014 dispatch a brief to the ${adapter.name} CLI and capture the run.
